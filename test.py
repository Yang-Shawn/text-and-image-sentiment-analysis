...
#第一个卷积层，32个卷积核，每个卷积核大小3*3。1表示输入的图片的通道,灰度图为1通道。
...
#validation_data : 被用来作为验证数据的(X, y)元组。
...
test = array([array(list2[i]).flatten() for i in range(200)],'f')
test = test.reshape(test.shape[0], img_rows, img_cols, 1)
test = test.astype('float32')
test /= 15
y_ss=[[0 for col in range(2)] for row in range(200)] 
text=np.loadtxt("text.txt")
yss = model.predict(test)
np.savetxt("image.txt",yss)
#print("image::",yss)
for i in range(200):
  for j in range (2):
    y_ss[i][j]=(text[i][j]+yss[i][j])/2
  #print("text::",text[i][0],text[i][1])
print("image&text::",y_ss)
y_ss = np.argmax(y_ss, axis=1)
print(y_ss)
z=0
k=0
for i in range(100):
